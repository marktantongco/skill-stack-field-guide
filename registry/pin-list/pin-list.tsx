...
function PinListRow({
  item, isPinned, onToggle, onQuickUnpin, transition, className,
  pinIcon: PinIcon, unpinIcon: UnpinIcon, enableDragReorder,
}: RowProps) {
  ...
  const row = (
    <motion.div
      ...
      onContextMenu={(e) => { if (isPinned && onQuickUnpin) { e.preventDefault(); onQuickUnpin(); } }}
      transition={transition}
      role="button"
      tabIndex={0}
    ...
    >
      <div className="flex min-w-0 flex-1 items-center gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          {item.info && (
            <div className="truncate text-xs font-medium text-neutral-500 dark:text-neutral-400">{item.info}</div>
          )}
        </div>
      ...
      <div className={cn(
        ...
        isPinned ? 'bg-emerald-500 text-white shadow-sm shadow-emerald-500/30'
                 : 'bg-neutral-300 text-neutral-500 opacity-0 group-hover:opacity-100 dark:bg-neutral-700 dark:text-neutral-300',
      )}>
        {isPinned ? <PinIcon className="size-4 fill-current" /> : <PinIcon className="size-4" />}
      </div>
    </motion.div>
  );
  ...
  if (enableDragReorder) {
    return (
      <Reorder.Item value={item} as="div" className="list-none" whileDrag={{ scale: 1.03, zIndex: 50, boxShadow: '0 10px 30px rgba(0,0,0,0.15)' }}>
        {row}
      </Reorder.Item>
    );
  }
...
function PinList({
  ...
  className, itemClassName, labelClassName, pinnedSectionClassName, unpinnedSectionClassName, searchClassName,
  pinIcon = Pin, unpinIcon = PinOff,
  ...
  const update = React.useCallback((next: PinListItem[]) => {
    setListItems(next); onItemsChange?.(next);
  }, [onItemsChange]);
  ...
  const filtered = q
    ? listItems.filter((u) => u.name.toLowerCase().includes(q) || (u.info?.toLowerCase().includes(q) ?? false))
    : listItems;
      ...
      <LayoutGroup>
        {hasResults && pinned.length > 0 && (
          <section aria-label={labels.pinned} className={pinnedSectionClassName}>
            <AnimatePresence>
              <motion.p layout key="pinned-label" className={cn('mb-2 px-1 text-xs font-semibold uppercase tracking-wider text-neutral-500 dark:text-neutral-400', labelClassName)} {...LABEL_MOTION}>
                {labels.pinned} · {pinned.length}
              </motion.p>
            </AnimatePresence>
            <div className={cn('relative space-y-2', togglingGroup === 'pinned' ? 'z-5' : 'z-10')}>
              {enableDragReorder ? (
                <Reorder.Group axis="y" values={pinned} onReorder={(reordered) => { const rest = listItems.filter((u) => !u.pinned); update([...reordered, ...rest]); }} as="div" className="space-y-2">
                  {pinned.map((item) => (
                    <PinListRow key={item.id} item={item} isPinned onToggle={() => togglePin(item.id)} onQuickUnpin={enableQuickUnpin ? () => togglePin(item.id) : undefined} transition={transition} className={itemClassName} pinIcon={pinIcon} unpinIcon={unpinIcon} enableDragReorder />
                  ))}
                </Reorder.Group>
              ) : (
                pinned.map((item) => (
                  <PinListRow key={item.id} item={item} isPinned onToggle={() => togglePin(item.id)} onQuickUnpin={enableQuickUnpin ? () => togglePin(item.id) : undefined} transition={transition} className={itemClassName} pinIcon={pinIcon} unpinIcon={unpinIcon} enableDragReorder={false} />
                ))
              )}
            </div>
          </section>
        ...
        {hasResults && unpinned.length > 0 && (
          <section aria-label={labels.unpinned} className={unpinnedSectionClassName}>
            <AnimatePresence>
              <motion.p layout key="unpinned-label" className={cn('mb-2 px-1 text-xs font-semibold uppercase tracking-wider text-neutral-500 dark:text-neutral-400', labelClassName)} {...LABEL_MOTION}>
                {labels.unpinned} · {unpinned.length}
              </motion.p>
            </AnimatePresence>
            <div className={cn('relative space-y-2', togglingGroup === 'unpinned' ? 'z-5' : 'z-10')}>
              {enableDragReorder ? (
                <Reorder.Group axis="y" values={unpinned} onReorder={(reordered) => { const pinnedItems = listItems.filter((u) => u.pinned); update([...pinnedItems, ...reordered]); }} as="div" className="space-y-2">
                  {unpinned.map((item) => (
                    <PinListRow key={item.id} item={item} isPinned={false} onToggle={() => togglePin(item.id)} transition={transition} className={itemClassName} pinIcon={pinIcon} unpinIcon={unpinIcon} enableDragReorder />
                  ))}
                </Reorder.Group>
              ) : (
                unpinned.map((item) => (
                  <PinListRow key={item.id} item={item} isPinned={false} onToggle={() => togglePin(item.id)} transition={transition} className={itemClassName} pinIcon={pinIcon} unpinIcon={unpinIcon} enableDragReorder={false} />
                ))
              )}
            </div>
          </section>
